...
export default function ScannerStatus({ scanner, loading }) {
  ...
  const getStatusInfo = (scanner) => {
    ...
    const isStale = lastSeen && (Date.now() - new Date(lastSeen).getTime() > 30000);
  ...
  const stats = [
    { label: 'Expected', value: scanner?.scan_summary?.expected || 0, color: 'text-blue-400' },
    { label: 'Detected', value: scanner?.scan_summary?.received || 0, color: 'text-green-400' },
    { label: 'Sent', value: scanner?.scan_summary?.sent || 0, color: 'text-yellow-400' },
    { label: 'Irrelevant', value: scanner?.scan_summary?.irrelevant || 0, color: 'text-gray-400' }
  ];
  ...
  return (
    <div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className={`p-3 rounded-lg ${statusInfo.bgColor}`}>
              <StatusIcon className={`${statusInfo.color} ${scanner?.status === 'scanning' ? 'animate-pulse' : ''}`} size={24} />
            </div>
            <div>
      ...
      <div className="p-6">
        <h4 className="text-sm font-medium text-gray-400 mb-4">Current Scan Summary</h4>
